...
def get_default_xml_files():
    ...
    assert (os.path.isfile(location_xml_filename) and os.path.isfile(object_xml_filename) and os.path.isfile(
        question_xml_filename) and os.path.isfile(name_xml_filename))
...
class XMLKnowledgeBase(object):
    ...
    def query(self, logic, argument_tuple):
        if logic == "find_location(x)":
            ...
            if argument_tuple[0] == "heaviest" or argument_tuple[0] == "lightest" \
                    or argument_tuple[0] == "smallest" or \
                    argument_tuple[0] == "biggest":
